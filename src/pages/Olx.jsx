...
const Olx = () => {
  ...
  const handleFileChange = (e) => {
    ...
    newImageObjects.forEach((imgObj, i) => {
      updatedImages[emptyIndexes[i]] = imgObj;
    });
  ...
  const validateForm = () => {
    if (!type || !bhk || !bathrooms || !superBuiltUp || !carpetArea || !furnishing || !projectStatus || !listedBy || !parking || !facingside || !projectName || !adTitle || !description || !price || images.every(img => img === null) || (activetab === 'list' && !selectedState) || (activetab === 'current' && (!currentLocation.state || !currentLocation.city || !currentLocation.neighbourhood)) || !name) {
  ...
  return (
    <>
      ...
      <div className="mt-7 min-h-screen m-auto flex items-center justify-center flex-col mb-10">
        <h1 className="text-2xl text-center ">POST YOUR AD</h1>

        <div className="border  border-zinc-400 max-w-4xl  min-w-3xl">
          <div className="m-3">
            <h1 className="text-xl font-extrabold  border-zinc-400  ">
              SELECTED CATEGORY
            </h1>
            <p className="text-xs mt-5 flex gap-6  items-center">
      ...
      <div className="mt-1 flex gap-2">
        <button
          onClick={() => setBhk("1")}
          className={`cursor-pointer text-lg border px-6 rounded-sm border-zinc-400 py-1 hover:bg-blue-200 ${bhk === '1' ? 'bg-blue-100' : ''}`}
        >
          1
        </button>
        <button
          onClick={() => setBhk("2")}
          className={`cursor-pointer text-lg border px-6 rounded-sm border-zinc-400 py-1 hover:bg-blue-200 ${bhk === '2' ? 'bg-blue-100' : ''}`}
        >
          2
        </button>
        <button
          onClick={() => setBhk("3")}
          className={`cursor-pointer text-lg border px-6 rounded-sm border-zinc-400 py-1 hover:bg-blue-200 ${bhk === '3' ? 'bg-blue-100' : ''}`}
        >
          3
        </button>
        <button
          onClick={() => setBhk("4")}
          className={`cursor-pointer text-lg border px-6 rounded-sm border-zinc-400 py-1 hover:bg-blue-200 ${bhk === '4' ? 'bg-blue-100' : ''}`}
        >
          4
        </button>
        <button
          onClick={() => setBhk("4+")}
          className={`cursor-pointer text-lg border px-5 rounded-sm border-zinc-400 py-1 hover:bg-blue-200 ${bhk === '4+' ? 'bg-blue-100' : ''}`}
        >
          4+
        </button>
      </div>
      ...
      <div className="mt-1 flex gap-2">
        <button
          ...
          className={`cursor-pointer text-lg border px-6 rounded-sm border-zinc-400 py-1 hover:bg-blue-200 ${bathrooms === '1' ? 'bg-blue-100' : ''}`}
        >
          1
        </button>
        <button
          onClick={() => setBathrooms("2")}
          className={`cursor-pointer text-lg border px-6 rounded-sm border-zinc-400 py-1 hover:bg-blue-200 ${bathrooms === '2' ? 'bg-blue-100' : ''}`}
        >
          2
        </button>
        <button
          onClick={() => setBathrooms("3")}
          className={`cursor-pointer text-lg border px-6 rounded-sm border-zinc-400 py-1 hover:bg-blue-200 ${bathrooms === '3' ? 'bg-blue-100' : ''}`}
        >
          3
        </button>
        <button
          onClick={() => setBathrooms("4")}
          className={`cursor-pointer text-lg border px-6 rounded-sm border-zinc-400 py-1 hover:bg-blue-200 ${bathrooms === '4' ? 'bg-blue-100' : ''}`}
        >
          4
        </button>
        <button
          onClick={() => setBathrooms("4+")}
          className={`cursor-pointer text-lg border px-5 rounded-sm border-zinc-400 py-1 hover:bg-blue-200 ${bathrooms === '4+' ? 'bg-blue-100' : ''}`}
        >
          4+
        </button>
      </div>
      ...
      <div className="flex flex-col mt-6">
        ...
        <input
          className="w-130 border rounded-sm h-11 px-2"
          type="number"
          value={superBuiltUp}
        ...
        <input
          className="w-130 rounded-sm border h-11 px-2"
          type="number"
          value={carpetArea}
        ...
        <div className="mt-1 flex gap-2">
          <button
            ...
            className={`cursor-pointer text-lg border px-6 rounded-sm border-zinc-400 py-1 hover:bg-blue-200 ${parking === '1' ? 'bg-blue-100' : ''}`}
          >
            1
          </button>
          <button
            onClick={() => setParking("2")}
            className={`cursor-pointer text-lg border px-6 rounded-sm border-zinc-400 py-1 hover:bg-blue-200 ${parking === '2' ? 'bg-blue-100' : ''}`}
        >
            2
          </button>
          <button
            onClick={() => setParking("3")}
            className={`cursor-pointer text-lg border px-6 rounded-sm border-zinc-400 py-1 hover:bg-blue-200 ${parking === '3' ? 'bg-blue-100' : ''}`}
          >
            3
          </button>
          <button
            onClick={() => setParking("3+")}
            className={`cursor-pointer text-lg border px-5 rounded-sm border-zinc-400 py-1 hover:bg-blue-200 ${parking === '3+' ? 'bg-blue-100' : ''}`}
          >
            3+
          </button>
        </div>
        ...
        <select
          className="cursor-pointer w-130 rounded-sm border h-11 px-2"
          value={facingside}
        ...
        />
          <div className="w-130 text-xs text-gray-500 text-right mt-1">
        ...
        <textarea
          className="w-130 rounded-sm border h-24 px-2 py-1"
          value={description}
...
<div className="m-5">
      ...
      <div className="flex mt-5 gap-10 mb-4">
        <button
          className={`cursor-pointer w-55 mr-6 pb-1 ${
            activetab === "list"
      ...
      {activetab === 'list' &&(
       ...
       <h2 className="font-medium mt-4 text-base ">State*</h2>
                ...
                <select
                  className={`cursor-pointer w-130 rounded-sm border h-11 px-2 ${error ? 'border-red-500' : ''}`}
                  value={selectedState}
                  onChange={(e) => {
